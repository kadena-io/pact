...
singleModule :: Analyze.Request
singleModule = fromJust $ parseMaybe parseJSON $ (Object (HM.fromList
...
spec = around_ bracket $ describe "Servant API client tests" $ do
  ...
  it "correctly verifies a property" $ do
    ...
    let expt = fromJust $ parseMaybe parseJSON (Object (HM.singleton "output"
