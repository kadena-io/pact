...
spec_dbl_trans_pow = describe "dbl_pow" $ do
    ...
    it "pow 8.7 856739.34857" $ isInf (Dbl.dbl_pow 8.7 856739.34857)
...
spec_dbl_trans_sqrt = describe "dbl_sqrt" $ do
    it "sqrt -1" $ transIsNaN (Dbl.dbl_sqrt (-1))
...
spec_musl_trans_pow = describe "musl_pow" $ do
    ...
    it "pow 8.7 856739.34857" $ isInf (Musl.musl_pow 8.7 856739.34857)
...
spec_musl_trans_sqrt = describe "musl_sqrt" $ do
    it "sqrt -1" $ transIsNaN (Musl.musl_sqrt (-1))
...
spec_dec_trans_ln = describe "dec_ln" $ do
    it "ln 60" $ (fst <$> Dec.dec_ln defTGP 60) `shouldBeNum`
      4.09434456222210041431708305026404559612274169921875

spec_dec_trans_exp :: Spec
spec_dec_trans_exp = describe "dec_exp" $ do
    it "e" $ fmap fst (Dec.dec_exp defTGP 1) `shouldBeNum` 2.718281828459045

spec_dec_ln_exp :: Spec
spec_dec_ln_exp = describe "dec_ln . dec_exp" $ do
    prop "ln (exp x)" $ \x ->
      let (TransNumber (a, _)) = Dec.dec_exp defTGP x
      in fmap fst (Dec.dec_ln defTGP a) `roughlyEq` x

spec_dec_trans_pow :: Spec
spec_dec_trans_pow = describe "dec_pow" $ do
    let dpow a b = fst <$> Dec.dec_pow defTGP a b
    it "pow 2 0" $ dpow 2 0 `shouldBeNum` 1
    it "pow 2 1" $ dpow 2 1 `shouldBeNum` 2
    it "pow 2 2" $ dpow 2 2 `shouldBeNum` 4
    it "pow 2 3" $ dpow 2 3 `shouldBeNum` 8
    it "pow 2 4" $ dpow 2 4 `shouldBeNum` 16
    it "pow 2 10" $ dpow 2 10 `shouldBeNum` 1024
    it "pow 8.7 85.7" $ dpow 8.7 85.7 `shouldBeNum` 328700185683462282496121e57
    ...
    prop "pow x 0" $ \x -> dpow x 0 === TransNumber 1
    prop "pow x 1" $ \x -> dpow x 1 === TransNumber x
    prop "pow x 2" $ \x -> dpow x 2 `roughlyEq` (x * x)

spec_dec_trans_log :: Spec
spec_dec_trans_log = describe "dec_log" $ do
    let dlog a b = fst <$> Dec.dec_log defTGP a b
    it "log 2 1" $ dlog 2 1 `shouldBeNum` 0
    it "log 2 2" $ dlog 2 2 `shouldBeNum` 1
    it "log 2 4" $ dlog 2 4 `shouldBeNum` 2
    it "log 2 8" $ dlog 2 8 `shouldBeNum` 3
    it "log 2 16" $ dlog 2 16 `shouldBeNum` 4
    it "log 2 256" $ dlog 2 256 `shouldBeNum` 8
    it "log 2 1024" $ dlog 2 1024 `shouldBeNum` 10
    it "log 2 5" $ dlog 2 5 `shouldBeNum` 2.321928094887362
    it "log 13.7 721.8" $ dlog 13.7 721.8 `shouldBeNum` 2.5146170134618364

spec_dec_log_pow :: Spec
spec_dec_log_pow = describe "dec_log 2 . dec_pow 2" $ do
  prop "log 2 (pow 2 x)" $ \x ->
    let (TransNumber a) = fst <$> Dec.dec_pow defTGP 2 x
    in fmap fst (Dec.dec_log defTGP 2 a) `roughlyEq` x

spec_dec_trans_sqrt :: Spec
spec_dec_trans_sqrt = describe "dec_sqrt" $ do
    let dsqrt a = fst <$> Dec.dec_sqrt defTGP a
    it "sqrt -1" $ transIsNaN (dsqrt (-1))
    it "sqrt 1" $ dsqrt 1 `shouldBeNum` 1
    it "sqrt 4" $ dsqrt 4 `shouldBeNum` 2
    it "sqrt 9" $ dsqrt 9 `shouldBeNum` 3
    it "sqrt 16" $ dsqrt 16 `shouldBeNum` 4
    it "sqrt 25" $ dsqrt 25 `shouldBeNum` 5
    it "sqrt 36" $ dsqrt 36 `shouldBeNum` 6
    it "sqrt 49" $ dsqrt 49 `shouldBeNum` 7
    it "sqrt 2" $ dsqrt 2 `shouldBeNum` 1.4142135623730951
    prop "sqrt (x * x)" $ \x ->
      dsqrt (x * x) `roughlyEq` abs x

spec_dec_pow_sqrt :: Spec
spec_dec_pow_sqrt = describe "dec_pow (dec_sqrt x) 2" $ do
    prop "sqrt (x * x)" $ \(Positive x) ->
      let (TransNumber a) = fst <$> Dec.dec_sqrt defTGP x
      in (fst <$> Dec.dec_pow defTGP a 2) `roughlyEq` x

-- -------------------------------------------------------------------------- --
-- Utils
defTGP :: Dec.TransGasParams
defTGP =
  Dec.TransGasParams 0 0 (const (pure ()))

transIsNaN :: Show a => TransResult a -> Expectation
transIsNaN stmt = stmt `shouldSatisfy` (\case
  TransNaN _ -> True
  _ -> False)
