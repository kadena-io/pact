...
grantedCaps = S.union <$> getAllStackCaps <*> getAllManaged
  where
    getAllManaged = S.fromList . concatMap toList <$> getManagedCaps
...
capabilityInstalled cap = getManagedCaps >>= go . S.toList
  where
    go [] = return False
    go (mc:mcs) | matchManaged mc cap = return True
                | otherwise = go mcs

getManagedCaps :: Eval e (S.Set (ManagedCapability UserCapability))
getManagedCaps = use (evalCapabilities . capManaged) >>= \mm -> case mm of
  Nothing -> evalError def "Internal error: capabilities not initialized"
  Just mcs -> return mcs

updateManagedCaps :: ManagedCapability UserCapability -> Eval e ()
updateManagedCaps mc = do
  mcs <- getManagedCaps
  evalCapabilities . capManaged .= Just (S.insert mc mcs)
...
evalUserCapability i af scope cap cdef test = go scope
  where
    ...
    installManaged cs = mkMC >>= install
      where
        install mc = updateManagedCaps mc
...
checkManaged applyF cap@SigCapability{..} = getManagedCaps >>= go . S.toList
  where
    go [] = return Nothing
    ...
    check mc@ManagedCapability{..} rv = do
      ...
      updateManagedCaps $ set mcManaged newMgdValue mc
