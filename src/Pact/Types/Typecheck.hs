...
data Fun t =
  ...
  FDefun {
    ...
    _fArgs   :: !(V.Vector (Named t)),
    _fBody   :: !(V.Vector (AST t)),
    _fRetId :: !TcId
    }
  deriving (Eq,Functor,Foldable,Traversable,Show)
...
instance Pretty t => Pretty (Fun t) where
  ...
  pretty FDefun {..} = parensSep
    ...
    , indent 2 $ sep [ "(", indent 2 (sep (map pretty (toList _fArgs))), ")" ]
    , indent 2 $ vsep (map pretty (toList _fBody))
    , ""
    ]
...
data AST n =
  ...
  _aNode :: !n,
  _aAppFun :: !(Fun n),
  _aAppArgs :: !(V.Vector (AST n))
  } |
  Binding {
  _aNode :: !n,
  _aBindings :: !(V.Vector (Named n,AST n)),
  _aBody :: !(V.Vector (AST n)),
  _aBindType :: !(AstBindType n)
  } |
  List {
  _aNode :: !n,
  _aList :: !(V.Vector (AST n))
  } |
  Object {
  _aNode :: !n,
...
instance Pretty t => Pretty (AST t) where
  pretty a = case a of
     ...
     List {..} -> go "List"
       [ bracketsSep [ indent 2 $ vsep $ map pretty (toList _aList) ] ]
     Binding {..} -> go "Binding"
       [ parensSep
         [ pretty _aBindType
         , indent 2 $ vsep $ toList _aBindings <&> \(k,v) -> parensSep
           [ indent 2 $ pretty k <+> sep [ colon, indent 2 (pretty v) ]
           ]
         , indent 2 $ vsep $ map pretty (toList _aBody)
         ]
       ]
     App {..} -> go "App"
       [ indent 2 $ parensSep [ indent 2 $ vsep $ map pretty (toList _aAppArgs) ]
     ...
     ModRef{..} -> go "ModRef" [pretty _aModRefName, pretty (V.toList <$> _aModRefSpec)]
   where
     go :: Text -> [Doc] -> Doc
