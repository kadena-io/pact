...
mkTranslateEnv info args = TranslateEnv info nodeVars mempty 0
  where
    ...
    -- names:
    nodeVars = foldl'
      (\m (Arg (Unmunged nm) vid node _ety) ->
        Map.insert node (Munged nm, vid) m)
      Map.empty
      args
...
translateBinding (Named (Unmunged -> unmunged) node _) = do
  vid <- genVarId
  let munged = node ^. aId.tiName.to Munged
      info = node ^. aId . Pact.tiInfo
  varType <- translateType node
  pure $ Located info $ Binding vid unmunged munged varType
