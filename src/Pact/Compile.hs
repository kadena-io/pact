...
doModel exps namei i
  = TModel <$> traverse (doDefProperty namei) exps <*> pure i

doDefProperty:: Info -> Exp -> Compile (TPropertyDef Name)
doDefProperty namei exp = case exp of
  EList (ea@(EAtom "defproperty" Nothing Nothing _):rest) Nothing _ -> do
    ...
    let mkDef name args body = do
          ...
          pure $ TPropertyDef name args' body ai
    case rest of
