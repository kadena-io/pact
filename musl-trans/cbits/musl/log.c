...
static const struct log_data __log_data = {
...
static inline uint32_t kadena_top16(double x)
{
	return kadena_asuint64(x) >> 48;
}

double musl_log(double x)
{
	...
	top = kadena_top16(x);
#define LO kadena_asuint64(1.0 - 0x1p-4)
#define HI kadena_asuint64(1.0 + 0x1.09p-4)
	...
	if (kadena_predict_false(top - 0x0010 >= 0x7ff0 - 0x0010)) {
		/* x < 0x1p-1022 or inf or nan.  */
		if (ix * 2 == 0)
			return __math_divzero(1);
		if (ix == kadena_asuint64(INFINITY)) /* log(inf) == inf.  */
			return x;
