...
In this way, modrefs bring polymorphism to Pact. To programmers coming from
polymorphic languages like Javascript, Java or Python, module references may
look familiar. However, Pact is very much not object-oriented so there are
important differences in how polymorphism works and what it is best used for.
...
created within the Pact system.
...
be surprising. "Pinned modrefs" that bind eagerly to the module version is
being actively considered as an upcoming Pact change to extend the benefits of
dependency management to modrefs.
