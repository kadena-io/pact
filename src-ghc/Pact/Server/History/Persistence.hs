...
crToField :: CommandResult [TxLog A.Value] -> SType
crToField r = SText $ Utf8 $ BSL.toStrict $ A.encode r

crFromField :: ByteString -> (CommandResult [TxLog A.Value])
...
insertRow :: Statement -> (Command ByteString, CommandResult [TxLog A.Value]) -> IO ()
...
insertCompletedCommand :: DbEnv -> [(Command ByteString, CommandResult [TxLog A.Value])] -> IO ()
...
selectCompletedCommands :: DbEnv -> HashSet RequestKey -> IO (HashMap RequestKey (CommandResult [TxLog A.Value]))
