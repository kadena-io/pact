        ...
        return CommandExecInterface
          { _ceiApplyCmd = \eMode cmd ->
              applyCmd logger _ccEntity p gasModel
                blockHeight blockTime prevBlockHash spv eMode cmd (verifyCommand cmd)
          , _ceiApplyPPCmd = applyCmd logger _ccEntity p gasModel
                             blockHeight blockTime prevBlockHash spv }
  case _ccSqlite of
    Nothing -> do
...
applyExec rk hsh signers (ExecMsg parsedCode edata) = do
  ...
  EvalResult{..} <- liftIO $ evalExec signers defaultInterpreter evalEnv parsedCode
...
applyContinuation rk hsh signers cm = do
  ...
  EvalResult{..} <- liftIO $ evalContinuation signers defaultInterpreter evalEnv cm
  return $ resultSuccess _erTxId rk _erGas (last _erOutput) _erExec _erLogs
