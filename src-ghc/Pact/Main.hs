...
data Option =
  ...
  | OSignStdinReq { _oKeyFiles :: [FilePath], _oReqLocal :: Bool }
...
signStdinParser :: O.Mod O.CommandFields Option
signStdinParser = O.command "sign" $ O.info (OSignStdinReq <$> parser <*> localFlag <**> O.helper) i
  where
    parser = many $ O.strArgument
             ( O.metavar "KEY_FILE"
            <> O.help "File with a public / private key pair in the format generated by \"pact -g\"")
    i = O.fullDesc
        <> O.header synopsis
        <> O.progDesc synopsis
    synopsis = "Sign a transaction via. stdin."
